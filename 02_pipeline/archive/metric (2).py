...
# Refer to: https://www.kaggle.com/code/metric/svg-image-fidelity/notebook (V17)
...
class VQAEvaluator:
    ...
    def __init__(self):
        ...
        # self.model_path = kagglehub.model_download('google/paligemma-2/transformers/paligemma2-10b-mix-448')
        self.model_path = 'paligemma2-3b-mix-448'
    ...
    def score_batch(
        ...
        for i, (question, probs, correct_answer) in enumerate(zip(questions, batched_choice_probabilities, answers)):
            predicted_answer = max(probs.items(), key=lambda x: x[1])[0]
...
class AestheticEvaluator:
    def __init__(self):
        self.model_path = 'sac-logos-ava1-l14-linearmse\sac+logos+ava1-l14-linearMSE.pth'
        
        self.clip_model_path = 'clip-vit-large-patch14\ViT-L-14.pt'
    ...
    def score(self, image: Image.Image) -> float:
        ...
        score = score.item() / 10.0  # scale to [0, 1]
        return int(score * 10000) / 10000.0
        

    def clip_score(self, image: Image.Image, prompt: str) -> float:
        """Compute CLIP‐score (0–100) between one image and one prompt."""
        # 1) preprocess the image → tensor on GPU
        img_t = self.preprocessor(image).unsqueeze(0).to('cuda:0')
        # 2) tokenize the prompt → tensor on GPU
        txt_t = clip.tokenize([prompt], truncate=True).to('cuda:0')

        with torch.no_grad():
            # 3) encode
            img_feats = self.clip_model.encode_image(img_t)
            txt_feats = self.clip_model.encode_text(txt_t)
            # 4) normalize
            img_feats = F.normalize(img_feats, dim=-1)
            txt_feats = F.normalize(txt_feats, dim=-1)
            # 5) cosine similarity and scale to [0,100]
            sim = F.cosine_similarity(img_feats, txt_feats, dim=-1).item()
            score = max(sim * 100, 0.0)
...
s = score(solution.copy(), submission.copy(), 'row_id', random_seed=42)
s


# ### Test one instance score
...
score_instance(multiple_choice_qa, svg, random_seed=42)
...
query_template = 'Does <image> portray "{}" without any lettering? Answer yes or no.'
query_template = 'Does <image> portray "SVG illustration of {}" without any lettering? Answer yes or no.'
query = query_template.format(description)
print(query)
vqa_evaluator.score_yes_no(query, svg_to_png(svg))


# ### Test a few instances score

# In[12]:


s, score_df = score_instance([multiple_choice_qa] * 2, [svg] * 2)
s
